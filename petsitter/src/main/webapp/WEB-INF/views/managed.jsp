...
<div class="section-form">
	<div class="left-section">
		<div class="left-menu" type="button" onclick="location.href='/mypage';">
			<!-- OnClick="location.href ='http://주소'" -->
			Mypage
		</div>
      <div class="left-menu" type="button" id="completed">완료된 요청</div>
      <div class="left-menu change-page" type="button" id="managed">완료된 매니저 요청</div>
			...
			<section id="container">
				<form role="form" method="get">
					<table class="table table-hover">
						<thead>
							<tr><th>요청일</th><th>펫</th><th>요청자</th><th>자세히 보기</th></tr>
						</thead>
						
<!-- 목록 시작 -->
  		<c:forEach items="${managed}" var="managed">
   			<tr>
    		<td><fmt:formatDate pattern="yyyy-MM-dd"
									value="${managed.regdate }" /></td>
    		<td>${managed.pet}</td>
    		<td>${managed.writer}</td>
    		<td><button type="button" class="detailBtn" data-pet="${managed.pet }" data-content="${managed.content }"
    		data-payment="${managed.payment }" data-img="${managed.img }" data-writer="${managed.writer }">보기</button></td>
