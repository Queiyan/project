...
+(function ($) {
  ...
  var Time = function (hour, minute) {
    ...
    this.invert = function () {
      if (this.getT() === "오전") this.setHour(this.getHour() + 12);
  ...
  var MDTimePicker = function (input, config) {
    ...
    this.timepicker = {
      ...
      timeHolder: {
        ...
        am_pm: $('<span class="mdtp__ampm">오전</span>'),
      },
      clockHolder: {
        wrapper: $('<section class="mdtp__clock_holder"></section>'),
        am: $('<span class="mdtp__am">오전</span>'),
        pm: $('<span class="mdtp__pm">오후</span>'),
        clock: {
        ...
        buttonsHolder: {
          ...
          btnOk: $(
            `<span class="mdtp__button ok">${
              that.config.okLabel || "확인"
            }</span>`
          ),
          btnCancel: $(
            `<span class="mdtp__button cancel">${
              that.config.cancelLabel || "취소"
            }</span>`
          ),
        },
      },
    ...
    picker.clockHolder.am.click(function () {
      if (that.selected.getT() !== "오전") that.setT("am");
    });
    picker.clockHolder.pm.click(function () {
      if (that.selected.getT() !== "오후") that.setT("pm");
    });
  ...
  MDTimePicker.prototype = {
      ...
      for (var i = 0; i < 12; i++) {
        ...
        hour.find("span").click(function () {
          var _data = parseInt($(this).parent().data("hour")),
            ...
            _value =
              (_data +
                ((_selectedT === "오후" && _data < 12) ||
                (_selectedT === "오전" && _data === 12)
                  ? 12
                  : 0)) %
              24;
    ...
    setT: function (value) {
      ...
      this.timepicker.clockHolder.am[t === "오전" ? "addClass" : "removeClass"](
        "active"
      );
      this.timepicker.clockHolder.pm[t === "오후" ? "addClass" : "removeClass"](
        "active"
      );
    },
  ...
  $.fn.mdtimepicker.defaults = {
    timeFormat: "h:mm:ss.000", // format of the time value (data-time attribute)
